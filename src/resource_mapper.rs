...
pub struct ResourceMappingConfig {
    pub patterns: Vec<ResourceMappingPattern>,
    pub action_mappings: Vec<ActionMapping>,
}

#[derive(Debug, Deserialize)]
pub struct ResourceMappingPattern {
    pub pattern: String,
    pub resource_type: String,
    pub resource_id: Option<String>,
    pub parents: Option<Vec<ParentMapping>>,
    pub parameter_groups: HashMap<String, String>,
}
...
impl ResourceMapper {
    ...
    // Simple initialization function - call this from main
    pub async fn initialize(
        config_path: Option<&str>, 
        api_prefix_pattern: &str
    ) -> Result<(), Box<dyn std::error::Error>> {
        let mapper = if let Some(path) = config_path {
            info!("Loading resource mapping configuration from {}", path);
            Self::load_from_config(path, api_prefix_pattern).await?
        } else {
            info!("No resource mapping configuration file provided, using default mappings");
            create_default_resource_mapper(api_prefix_pattern)
        };

        // Replace the global mapper
        {
            let mut global_mapper = RESOURCE_MAPPER.write().await;
            *global_mapper = mapper;
        }

        info!("Resource mapping initialized successfully");
        Ok(())
    }

    // Get a reference to the global mapper
    pub async fn global() -> tokio::sync::RwLockReadGuard<'static, ResourceMapper> {
        RESOURCE_MAPPER.read().await
    }

    // Private function to load from config file
    async fn load_from_config(
        config_path: &str, 
        api_prefix_pattern: &str
    ) -> Result<ResourceMapper, Box<dyn std::error::Error>> {
        let config_str = fs::read_to_string(config_path)?;
        trace!("Resource mapping config content: {}", config_str);
        
        let config: ResourceMappingConfig = serde_json::from_str(&config_str)?;
        ...
        for mapping in &config.action_mappings {
            ...
            mapper.add_custom_action_mapping(
        ...
        debug!("Resource mapper configured with {} patterns", mapper.get_pattern_count());
    ...
    fn default_parse_path(&self, path: &str) -> Result<ResourcePath, ResourceMappingError> {
        ...
        3 => {
            Ok(ResourcePath {
                ...
                parameters: HashMap::new(),
                matched_pattern: format!("default:/{}/{{id}}/{}", segments[0], segments[2]),
            })
        },
        ...
        4 => {
            Ok(ResourcePath {
                ...
                parameters: HashMap::new(),
                matched_pattern: format!("default:/{}/{{id}}/{}/{{id}}", segments[0], segments[2]),
            })
        },
